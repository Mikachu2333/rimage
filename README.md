...
- PSD is partially supported; images with embedded ICC profiles may still produce incorrect colors when the profile or pixel layout is unsupported.
- Animated WebP frame timing is a known limitation of the current `webp` wrapper. Encoded and decoded frame durations, especially the final frame duration, may be inaccurate.
